...
# Pre-create some existing WP directories with various states
mkdir -p 1ABC_WP && echo "test" > 1ABC_WP/west.h5 && echo "#!/bin/bash" > 1ABC_WP/run.slurm && chmod +x 1ABC_WP/run.slurm

# Mock h5ls for iteration counts
h5ls() {
    case "$1" in
...
# Run the real scripts directly (use their shebang/interpreter)
